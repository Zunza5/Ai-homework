...
##🛠️ RequirementsThe project is managed via **PDM** and requires:

* Python **3.11**
* **z3-solver** (>=4.15.4.0)

##🚀 InstallationEnsure you have [PDM](https://pdm.fming.dev/) installed on your system.
...
##💻 UsageYou can run the different solvers directly via Python (or via `pdm run`).

###Run the CSP Solver (Z3)This script solves the problem for **N=20** (default) using the Z3 solver.
...
###Run the A* SolverThis script solves the problem for **N=8** (default), showing the algorithm's steps.
